...
Page({
    ...
    loadData() {
        ...
        //  调用后台接口
        api.getWxjcjgbWorkList(params).then(responseData => {
            wx.hideLoading();
    ...
    onRowClick(e) {
        ...
        // 跳转到detail页面，传递必要参数
        // 在当前页面保存数据到缓存
        const cacheKey = 'temp_wxjc_item_' + Date.now();
        wx.setStorageSync(cacheKey, item);
        console.log("跳转的对象:", item);
        wx.navigateTo({
            url: `/pages/wxjcDetail/wxjcDetail?cacheKey=${cacheKey}`,
        });
    },
    ...
    showEditModal(index, item) {
        const _this = item.jjwxjcb.jjcpbgl.cpmc + '   ' + item.jjwxjcb.jjcpbgl.cpxh
        const rq = item.rq;
    ...
    showDeleteConfirm(index, item) {
        wx.showModal({
            title: '确认删除',
            content: `确定要删除 "${item.jjwxjcb.jjcpbgl.cpmc}  ${item.jjwxjcb.jjcpbgl.cpxh}" 的工序表吗？`,
            confirmText: '删除',
            confirmColor: '#ff4444',
            cancelText: '取消',
            success: (res) => {
    ...
    deleteRecord(id) {
        ...
        //  调用后台接口
        api.deleteWxjcjgbWork(saveData).then(responseData => {
            wx.hideLoading();
            wx.showToast({
                title: '删除成功',
                icon: 'success',
    ...
    saveEditInfo() {
        ...
        //  调用后台接口
        api.updateWxjcjgbWork(jjgxjcbEntity).then(responseData => {
            wx.hideLoading();
            wx.showToast({
                title: '修改成功',
                icon: 'success',
